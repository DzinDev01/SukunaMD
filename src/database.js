	...
	DataBase = class mongoDB {
		constructor(url = global.tempatDB, options = { useNewUrlParser: true, useUnifiedTopology: true, serverSelectionTimeoutMS: 5000 }) {
			...
			mongoose.connection.on('disconnected', async () => {
				...
				console.warn('MongoDB koneksi terputus, sedang menghubungkan ulang selama 5 detik')
		...
		connect = async (retries = 5, delay = 2000) => {
			if (mongoose.connection.readyState === 1 || this.isConnecting) {
				console.log('MongoDB udah aktif')
				return
			}
			this.isConnecting = true
			while (retries > 0) {
				try {
					console.log(`🔄 Sedang menghubungkan mongoDB (Attempt ${6 - retries}/5)`)
					...
					console.error(`Gagal menyambung mongoDB ${e.message}`)
					await new Promise((res) => setTimeout(res, delay))
					retries--
				}
			}
			this.isConnecting = false
			throw new Error('MongoDB gagal mengkoneksi')
		}
